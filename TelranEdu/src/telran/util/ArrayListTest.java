...
void setUp() {
	...
	void testSortPersonsByAge() {
		...
		Person expected[] = {p3, p1, p2};
			...
			//using Lambda - realization the compare method of comparator interface
		persons.sort((prs1, prs2) -> Integer.compare(prs2.getAge(), prs1.getAge()));
			//using comparator - separated class
	...
	void testEvenOddSorting() {
		...
		list.add(-17);
			...
			//using class separated comparator:
		 ...
		 */		
			...
			//Lambda: expression using separate function
	...
	@Test
 	void testIndexOfPredicate()  {
 		...
 		list.add(-17);
 		//there is no such element in array => should return -1
